...
class Ganalytics extends Module
{
    ...
    */
    public function hookOrderConfirmation($params)
    {
        ...
        if (Validate::isLoadedObject($order))
        {
            ...
            if (true/*$ga_order_sent === false && $order->id_customer == $this->context->cookie->id_customer*/)
            {
                ...
                $this->js_state = 1;
                 $this->_debugLog('orderconfirmation', $this->_runJs($ga_scripts));
    ...
    */
    public function addTransaction($products, $order)
    {
        ...
        return $js.'MBG.addTransaction('.Tools::jsonEncode($order).','.Tools::jsonEncode($products).');';
    }
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (!empty($ga_account_id))
        {
            ...
            if ($this->context->controller->controller_name == 'AdminOrders' && !(Tools::getValue('id_order') >0))
            {
                ...
                $ga_order_records = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'ganalytics` WHERE sent = 0 AND DATE_ADD(date_add, INTERVAL 30 minute) < NOW() GROUP BY id_order HAVING MIN(id_google_analytics)');
